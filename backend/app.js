...
async function analyzeDataWithGemini(data) {
    try {
        const prompt = `Analyze the following inventory data and provide key insights, potential issues, and recommendations: ${JSON.stringify(data)}`;//
        ...
        return {
            table: {
                ...
                body: [
                    ...
                    // Data rows
                    ...data.map(item => headers.map(header => item[header]?.toString() || ''))
                ]
            },
            layout: {
...
app.post("/api/v1/inventory/generate-analysis-report", async (req, res) => {
    try {
        ...
        if (!inventoryData || !Array.isArray(inventoryData) || inventoryData.length === 0) {
            return res.status(400).json({ error: "Inventory data is required in the request body." });
        }
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="inventory_analysis_report.pdf"');
        res.send(pdfBuffer);

    } catch (error) {
        console.error("Error generating inventory analysis report:", error);
        res.status(500).json({ error: 'Failed to generate inventory analysis report', details: error.message });
    }
});
