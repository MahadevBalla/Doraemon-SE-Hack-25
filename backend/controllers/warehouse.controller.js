...
export const updateWarehouseQuantity = async (warehouseId, quantityChange) => {
  const warehouse = await Warehouse.findById(warehouseId);
  if (!warehouse) throw new Error("Warehouse not found");
  warehouse.quantity += quantityChange;
...
export const getAllWarehouses = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, warehouses, "Fetched all warehouses"));
});
